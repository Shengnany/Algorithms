...
public class mergeX {
    ...
    private static void sort(Object[] src, Object[] dst, int lo, int hi, Comparator comparator) {
        ...
        int mid = lo + (hi - lo) / 2;
        sort(dst, src, lo, mid, comparator); //分开sort src (base case 1 element)
        ...
        merge(src, dst, lo, mid, hi, comparator); // mergesort src into dst  base case(sort dst)
    }
    //dst ok, 上一级的src sorted
    //总结来说， sort 满足第一个argument sort。如何做到：把下一层的des sort了，然后mergeSort src。
    //如何省略copy：把a作为目标地，把aux作为操作地。recursive时，每一步都通过分开sort aux而mergeSort a
    //每次sort 完：dst sorted 了，即上一步的src sorted了，再把上一步的dst merge。 上上步的src，sort完成了
